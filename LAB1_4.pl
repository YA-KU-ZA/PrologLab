...
problem(keyboard_error):-
    query('Is the keyboard not responding to key presses').

problem(ram_error):-
    query('Is the system slowing down or crashing unexpectedly').
...
% образом, чтобы их можно было однозначно определить без введени¤
% дополнительных вопросов, с введением дополнительных вопросов.
